...
- `WindowManipulator.swift` — Uses `AXUIElementCreateApplication(pid)` + `kAXPositionAttribute`/`kAXSizeAttribute` to move and resize windows via the Accessibility API. When the target window belongs to Gridwell's own process, uses `NSWindow.setFrame` (AppKit) instead — the AX API cannot manipulate the calling app's own windows. A `DispatchSourceTimer` (10 Hz) on a background serial queue applies the latest pending frame. Each frame is applied via a `setFrame` helper that sets **size before position** (to prevent the system from repositioning the window after a size change) and uses a **bounded read-back retry loop** (up to 3 attempts, 80 ms time budget, stale-frame abort) that verifies **both position and size** before declaring success. `AXUIElementSetMessagingTimeout` is called with 0.5 s at drag start so a momentarily unresponsive app can't block the serial queue for the system default (several seconds). `AXEnhancedUserInterface` is temporarily disabled for the drag session (restored in `endDrag`) to prevent animated window moves in Chromium/Electron apps that would cause read-back mismatches and excessive retries — the documented workaround used by Rectangle and Nudge.
...
- **Resize zones**: any of the four edges when the click is within `resizeBorderWidth` points of that edge (configurable, default 150 pt); clamped to 40 % of the relevant window dimension on small windows. Corners activate both adjacent edges simultaneously. Centre clicks are move.
...
14. ✅ Unreleased refinements for next release
  ...
  - **Custom menu bar icon**: replaced the SF Symbol `rectangle.3.group` with a custom `MenuExtraIcon` PNG image asset. The icon is stored in `Assets.xcassets/MenuExtraIcon.imageset/` and referenced via `image: "MenuExtraIcon"` in the `MenuBarExtra` scene.
  - **Mouse button trigger**: windows can now be dragged and resized by pressing a configurable extra mouse button (middle, back, forward, etc.) without any modifier key. The button is chosen via a click-to-record row in the Keys preferences tab (same card pattern as the keyboard shortcut recorder). Setting stored as `mouseButtonTrigger: Int?` in `GridConfigStore` (nil = disabled). The CGEventTap intercepts `otherMouseDown/Dragged/Up`; a `DragSource` enum tracks whether the active drag was started by the modifier key or the mouse button so both triggers coexist independently.
  - **Mouse button app exceptions**: a per-app exclusion list for the mouse button trigger. In listed apps the configured mouse button is passed through untouched so native behaviour (e.g. middle-click auto-scroll in browsers) is preserved. Configured via a new "Mouse Button Exceptions" card in the Keys tab — click "+" to pick any `.app` from an NSOpenPanel, "−" to remove. Stored as a JSON array of `ExcludedApp` structs (bundle ID + display name) in `UserDefaults`. The exclusion check in `MouseInteractionHandler` does a lightweight `CGWindowListCopyWindowInfo` query on each `otherMouseDown` to identify the topmost window at the cursor and resolve its pid to a bundle identifier via `NSRunningApplication`.
  - **Grid preview fills card width**: the `clipShape`+`strokeBorder` overlay is now applied directly to the `aspectRatio` view (before the frame modifier), so the border wraps the actual grid area instead of a full-width transparent container. The frame is changed from `maxWidth: 250, maxHeight: 108` (two frames) to a single `frame(maxWidth: .infinity, maxHeight: 200)`. For ultra-wide screens (e.g. 5120 × 2160, ratio 2.37:1) the grid fills the full card width with equal 16 pt padding on all sides; for standard monitors the grid is taller and wider than before and remains centred.
  - **AX resize degradation fix**: window dragging/resizing no longer degrades over time or with fast mouse movements. Three changes to `WindowManipulator`: (1) `AXUIElementSetMessagingTimeout` (0.5 s) bounds each AX IPC call so an unresponsive target app can't block the serial timer queue for the system default (several seconds); (2) `AXEnhancedUserInterface` is temporarily disabled for the drag session (restored in `endDrag`) to prevent animated window moves in Chromium/Electron apps that cause read-back mismatches and excessive retries — the documented workaround used by Rectangle and Nudge; (3) the `setFrame` retry loop is bounded to 3 attempts with an 80 ms time budget and a stale-frame abort (between retries, if a newer `pendingFrame` arrived from the drag handler, the current frame is abandoned instead of burning IPC calls on an obsolete frame). Worst-case IPC calls per tick drop from 20 to 12, and fast mouse movements no longer cause the queue to back up.
